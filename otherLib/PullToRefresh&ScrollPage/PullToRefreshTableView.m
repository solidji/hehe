...
- (void)changeState:(int)state{
    ...
    switch (state) {
        ...
        case k_PULL_STATE_NONE:
            currentState = k_PULL_STATE_END;
            stateLabel.text = viewType == k_VIEW_TYPE_HEADER ? stateLabel.text : @"暂无数据";
            arrowView.hidden = YES;
            break;
...
-(id)initWithFrame:(CGRect)frame withType:(PullToRefreshTableViewType)aType
{
    ...
    if (self) {
        ...
        else if(tableType == withoutArrow)//移除箭头和上拖加载
        {
            headerView = [[StateView alloc] initWithFrame:CGRectMake(0, -40, frame.size.width, frame.size.height) viewType:k_VIEW_TYPE_HEADER];
            [headerView.arrowView removeFromSuperview];
            [headerView.indicatorView removeFromSuperview];
            
            footerView = [[StateView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height) viewType:k_VIEW_TYPE_FOOTER];
            [footerView.arrowView removeFromSuperview];
            [footerView.indicatorView removeFromSuperview];
            [self addSubview:headerView];
            //[self setTableFooterView:footerView];
        }
        else
